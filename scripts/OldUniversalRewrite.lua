...
local function requesturl(url, bypass) 
    ...
    local repourl = bypass and "https://raw.githubusercontent.com/githubuser2243/" or "https://raw.githubusercontent.com/githubuser2243/Future/main/"
...
local AutoClickerMode = {["Value"] = "Real"}
local AutoClickerSpeed = {["Value"] = 1}
local AutoClickerRandomSpeed = {["Value"] = 1}
local AutoClicker = {["Enabled"] = false}; AutoClicker = GuiLibrary["Objects"]["CombatWindow"]["API"].CreateOptionsButton({
    ["Name"] = "AutoClicker",
    ["Function"] = function(callback) 
        spawn(function()
            repeat wait()
                local clickfunc = AutoClickerMode["Value"] == "Real" and mouse1click or AutoClickerMode["Value"] == "VU" and function() game:GetService("VirtualUser"):Button1Down() game:GetService("VirtualUser"):Button1Up() end or AutoClickerMode["Value"] == "Tool" and isAlive() and lplr.Character:FindFirstChildOfClass("Tool") and function() lplr.Character:FindFirstChildOfClass("Tool"):Activate() end
                if clickfunc and autoclickertick <= tick() and UIS:IsMouseButtonPressed(0) then 
                    autoclickertick = tick() + ((1 / AutoClickerSpeed["Value"]) + (math.random(1, AutoClickerRandomSpeed["Value"]) / 50))
                    clickfunc()
                end
            until AutoClicker.Enabled==false
        end)
    end
})
AutoClickerMode = AutoClicker.CreateSelector({
    ["Name"] = "Mode",
    ["Function"] = function() end,
    ["List"] = {"Real", "VU", "Tool"} 
})
AutoClickerSpeed = AutoClicker.CreateSlider({
    ["Name"] = "Speed", 
    ["Function"] = function() end,
    ["Min"] = 1,
    ["Max"] = 20,
...
AutoClickerRandomSpeed = AutoClicker.CreateSlider({
    ["Name"] = "RandomSpeed", 
    ["Function"] = function() end,
    ["Min"] = 1,
    ["Max"] = 10
...
BindToStepped("PlayerLoop", function() 
    for i,v in next, PLAYERS:GetPlayers() do 
        -- loop without local player
        if isAlive(v) and v~=lplr then 
            if Reach["Enabled"] then
                local part = ReachHitBoxesPart.Value == "Root" and "HumanoidRootPart" or nil
                if part and v.Character:FindFirstChild(part) then
                    v.Character:FindFirstChild(part).Size = Vector3.new(ReachHitBoxes.Value + 2, ReachHitBoxes.Value + 2, ReachHitBoxes.Value + 2)
                end
            end
            if CHAMS["Enabled"] then 
                if (CHAMSTeamCheck["Enabled"] and v.Team ~= lplr.Team or not CHAMSTeamCheck["Enabled"]) then
                    local highlight
